...
class BNN(object):
    ...
    def model(self, x_data, y_data):
        ...
        # lift：事前分布を導入した場合の生成モデルを定義する
        lifted_module = pyro.random_module("module", self.net, priors)
        lifted_bnn_model = lifted_module()
        # print(lifted_bnn_model)
        # ↓↓↓↓↓↓ 出力結果 ↓↓↓↓↓↓
        # Net(
        #   (fc1): NonBiasLinear()
        #   (fc2): NonBiasLinear()
        ...
        with pyro.plate("map", len(x_data)):
            # squeeze()を使わないとprediction_meanのshapeは(15, 1)，使うと(15)
            # squeeze()：次元削除する
    ...
    # MAP推定
    def MAPestimation(self, x_data, y_data, num_iterations=1000):
        guide = AutoDelta(self.model)
        svi = SVI(self.model, guide, Adam({"lr": 1e-3}), loss=Trace_ELBO())

        # train
        # Pyroではグローバル変数に現在の計算グラフの各変数を保存しているため，
        # 実際に最適化を実行する前に一度だけclear_param_store関数で初期化する
        ...
        # print(param_dict)
        w1_MAP = param_dict['AutoDelta.module$$$fc1.weight']
        w2_MAP = param_dict['AutoDelta.module$$$fc2.weight']
